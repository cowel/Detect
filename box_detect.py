...
def box_extraction(img_for_box_extraction_path, result_dir_path):
    ...
    df2 = pd.DataFrame(data = ARs)
    

    writer = pd.ExcelWriter(result_dir_path, engine='xlsxwriter')
    df.to_excel(writer, sheet_name='Sheet1',header = False)
    df2.to_excel(writer, sheet_name='Sheet2',header = ["Kết quả"], startcol=13, startrow = 6,index = False)
    workbook  = writer.book
    # for col in (0, 2):
    #     writer.sheets['Sheet2'].set_column(col, col, 15, my_format)
    # workbook.add_sheet("Sheet2") 
    worksheet = writer.sheets['Sheet2']
    worksheet.conditional_format('N8:N43', {'type': '3_color_scale'})
    # img = Image.open('photo.jpg')
    # img = openpyxl.drawing.image.Image("0001.jpg")    # Insert an image.
    # my_png = openpyxl.drawing.image.Image(png_loc)
    worksheet.insert_image("A1",img_for_box_extraction_path)
    
    # data_format1 = workbook.add_format({'bg_color': '#FFC7CE'})
    # data_format2 = workbook.add_format({'bg_color': '#00C7CE'})
    # worksheet.conditional_format('O11:O50', {'type': '3_color_scale'})
    # worksheet.conditional_format('N8:N43', {'type': '3_color_scale',
    #                                      'min_color': "#C5D9F1",
    #                                      'mid_color': "#8DB4E3",
    #                                      'max_color': "#538ED5"})
    writer.save()
box_extraction("0001.jpg", '/home/phong/Documents/NhanDien/BoxDetection/example.xlsx')
